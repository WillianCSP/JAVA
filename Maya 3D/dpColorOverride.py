...
#
#    date:
#        v 1.0 _ 2012-09-22 - first release
#
###################################################################
...
def dpColorShape(objList, color):
...
def dpSetColorToSelect(colorIndex, *args):
	""" Get the selection list and set the color to each shape
	"""
	selList = cmds.ls(selection=True)
	dpColorShape(selList, colorIndex)
...
def dpColorizeUI():
	...
	for colorIndex, colorValues in enumerate(colorList):
		cmds.button('indexColor_'+str(colorIndex)+'_BT', label=str(colorIndex), backgroundColor=(colorValues[0], colorValues[1], colorValues[2]), command=partial(dpSetColorToSelect, colorIndex), parent=colorIndexLayout)
